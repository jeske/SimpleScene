...
namespace SimpleScene
{
    public static class SSTexturedCube
    {
        ...
        private static readonly SSVertex_PosTex1[] c_vertices = {
            ...
            new SSVertex_PosTex1(-.5f, -.5f, +0.5f, 0f, 1f),
            new SSVertex_PosTex1(+.5f, -.5f, +0.5f, 1f, 1f),
            new SSVertex_PosTex1(+.5f, +.5f, +0.5f, 1f, 0f),

            new SSVertex_PosTex1(-.5f, -.5f, +0.5f, 0f, 1f),
            new SSVertex_PosTex1(+.5f, +.5f, +0.5f, 1f, 0f),
            new SSVertex_PosTex1(-.5f, +.5f, +0.5f, 0f, 0f),
        };
        ...
        static SSTexturedCube()
        {
            ...
            SSVertex_PosTex1[] vertices = new SSVertex_PosTex1[verticesPerQuad * 6];
            for (int f = 0; f < 6; ++f) {
                for (int v = 0; v < verticesPerQuad; ++v) {
                    SSVertex_PosTex1 vertex = c_vertices [v];
                    vertex.Position = Vector3.Transform(vertex.Position, c_faceTransforms [f]);
                    vertices [f * verticesPerQuad + v] = vertex;
                }
            }
            Instance = new SSVertexBuffer<SSVertex_PosTex1> (vertices);
        }
    }
}
